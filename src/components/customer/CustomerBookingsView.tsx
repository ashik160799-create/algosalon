...
export const CustomerBookingsView: React.FC = () => {
  ...
  const handleRebook = (apt: Appointment) => {
    ...
    const service = services.find(s => s.id === apt.serviceId && s.salonId === salon.id) ||
      services.find(s => s.salonId === salon.id) ||
      null;
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-1.5 pt-0.5 flex-wrap">
                    ...
                    {(() => {
                      const salon = salons.find(s => s.id === apt.salonId);
                      const dual = getDualBookingTime(apt.date, apt.timeSlot, salon, activeCountry.code);
                      if (dual && !dual.isSameTimezone) {
                        return (
                          <span className="text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">
                            {dual.salonTzCode} • Local: {dual.customerFormatted.split('__')[1] || dual.customerTzCode}
                          </span>
                        );
                      }
                      return null;
