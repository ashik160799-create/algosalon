-- =============================================================================
-- ALGO SALON SPOT-PRO: Complete Core Schema, Access Control & Safeguarded Procedures
-- Worldwide-ready Salon Management & Booking Platform
-- =============================================================================

begin;

-- Extensions required for UUID generation and double-booking exclusion constraints
...
-- 2.3 Salon Members (Access Control: Owner, Manager, Staff)
...
-- Guarantees at the database engine level that no stylist can be double booked for active slots
...
-- 2.11 Appointment Events (Immutable Audit Log)
...
-- 2.15 Payment Records (Stripe/Payment Provider Isolation - strictly protected)
...
-- -----------------------------------------------------------------------------
-- 3. Helper Functions & Security Checks
-- -----------------------------------------------------------------------------

-- Check if current authenticated user is an active member of a specific salon
...
-- 4.15 Payment Records RLS (Deny all direct browser access - accessible only via service role)
...
-- -----------------------------------------------------------------------------
-- 5. Stored Procedures (Guarded RPCs for Booking & Reviews)
-- -----------------------------------------------------------------------------

-- 5.1 Safe Atomic Booking Creation
...
begin
  -- Validate service existence and fetch official duration & price
  ...
  -- Validate staff existence and bookability
  ...
  -- Derive accurate end time from official duration
  v_ends_at := p_starts_at + (v_service.duration_minutes || ' minutes')::interval;

  -- Ensure appointment starts in the future
  if p_starts_at <= now() then
    raise exception 'Appointment time must be in the future';
  end if;

  -- Check conflict (double-booking protection)
  if exists (
...
begin
  ...
  -- Customers can only cancel or accept suggested times
...
-- Storage policies: anyone can read avatars & salon media
...
-- Automatically syncs auth.users -> public.profiles on sign up
-- -----------------------------------------------------------------------------
