-- =============================================================================
-- Migration: 20260906_000003_audit_security_and_booking_fixes.sql
-- Description: Comprehensive fix for security, RLS, booking validation,
--              timezones, business registration, notifications, reviews, and
--              data synchronization based on the full app audit.
-- =============================================================================

begin;

-- -----------------------------------------------------------------------------
-- 1. Profiles Table Updates: Email, Role & Enhanced Registration Sync
-- -----------------------------------------------------------------------------
alter table public.profiles
  add column if not exists email text,
  add column if not exists role text not null default 'customer' check (role in ('customer', 'business'));
...
-- Enhanced user registration trigger that captures email and role
...
-- -----------------------------------------------------------------------------
-- 2. Salons Table Updates: Persistent isOpenNow Status
-- -----------------------------------------------------------------------------
alter table public.salons
  add column if not exists is_open_now boolean not null default true;

-- -----------------------------------------------------------------------------
-- 3. Special Schedules Table (Authoritative Multi-Device Schedule Overrides)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 4. Strict Profiles RLS (Removes broad authenticated-user leakage)
-- -----------------------------------------------------------------------------
...
create policy "Profiles are viewable by owner or staff"
  ...
  using (
    ...
    auth.uid() = id
    -- Salon staff/managers can view profiles of clients who have booked at their salon
    or exists (
      select 1
    ...
    -- Or team members of the same salon
    or exists (
      select 1
...
-- -----------------------------------------------------------------------------
-- 5. Strict Appointments RLS (Removes broad authenticated-user leakage)
-- -----------------------------------------------------------------------------
...
-- Direct client updates are strictly limited: customers can ONLY cancel their own appointment
...
create policy "Users can insert notifications for appointments"
  ...
  with check (
    -- User creates notification for themselves or for an appointment participant
    user_id = auth.uid()
    or (
...
-- -----------------------------------------------------------------------------
-- 7. Scoped Storage Upload Policies (Prevents overwriting other users/salons)
-- -----------------------------------------------------------------------------
...
-- Set bucket file size limits if supported
update storage.buckets
set file_size_limit = 5242880, -- 5 MB for avatars
    allowed_mime_types = array['image/jpeg', 'image/png', 'image/webp', 'image/jpg']
where id = 'avatars';

update storage.buckets
set file_size_limit = 10485760, -- 10 MB for salon media
    allowed_mime_types = array['image/jpeg', 'image/png', 'image/webp', 'image/jpg']
where id = 'salon-media';

-- -----------------------------------------------------------------------------
-- 8. Upgraded create_booking RPC
--    Validates: staff-service mapping, salon operating hours, special holiday
--    closures, staff working hours, future time, duration against closing time,
--    and creates real-time notifications for salon managers.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 9. Upgraded set_appointment_status RPC
--    Validates: strict state transitions, reschedule times, locks customer
--    confirmation to business proposed time, and creates cross-device notifications.
-- -----------------------------------------------------------------------------
...
begin
  ...
  if v_is_manager and v_apt.customer_id is not null then
    ...
    -- Customer updated (e.g. cancelled or accepted reschedule): notify salon managers
    for v_owner in (
      select user_id from public.salon_members
...
-- -----------------------------------------------------------------------------
-- 10. Guarded submit_review RPC
--     Derives customer identity, service, and specialist directly from the
--     verified completed appointment in the database.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 11. Guarded register_business_salon RPC
--     Atomically establishes auth -> profile -> salon -> salon_members ('owner')
-- -----------------------------------------------------------------------------
...
begin
  ...
  -- 4. Create default weekly business hours (Monday-Sunday 09:00 - 21:00)
...
-- -----------------------------------------------------------------------------
-- 12. Authoritative delete_user_account RPC
-- -----------------------------------------------------------------------------
...
begin
  ...
  -- Cascades delete to profiles, salon_members, appointments, favorites, reviews, notifications
